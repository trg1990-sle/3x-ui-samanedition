...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/trg1990-sle/3x-ui-samanedition/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/trg1990-sle/3x-ui-samanedition/main/x-ui.sh
